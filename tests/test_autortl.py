...
def test_autortl_fires_after_5min_drift_once(vault):
    ...
    assert "redteamlead" in o1.get("additionalContext", "").lower()
    o2 = _run("bash /root/vm.sh 'curl http://10.0.0.5/'", env)   # fires ONCE per window

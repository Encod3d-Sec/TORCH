...
def test_expected_hooks_trimmed():
    ...
    assert len(ch.EXPECTED_HOOKS) == 12            # -web-recon (auto-launched recon on discovery); +drift-guard
...
def test_wiki_log_removed_and_count_is_12():
    import importlib.util, os
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    spec = importlib.util.spec_from_file_location("ch", os.path.join(root, "scripts", "check-hooks.py"))
    ch = importlib.util.module_from_spec(spec); spec.loader.exec_module(ch)
    ...
    assert len(ch.EXPECTED_HOOKS) == 12   # ... +tool-telemetry +wiki-reindex, -web-recon, +drift-guard
...
def test_docs_hook_count_matches_expected():
    ...
    assert m and int(m.group(1)) == len(ch.EXPECTED_HOOKS) == 12

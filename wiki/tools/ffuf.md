...
- **A WAF can 403 the ffuf User-Agent for every path** (Cloudflare does this by default), which turns the whole run into "matches" and hides the real routes. ffuf sends `Fuzz Faster U Fool <version>` unless told otherwise. Sanity-check ONE known-404 path with the scanner UA vs a browser UA before trusting any result set, and pass `-H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36'` on WAF-fronted targets. Same applies to feroxbuster/nuclei ([[feroxbuster]], [[wiki/tools/nuclei]]).
